...
if __name__ == '__main__':
	...
	# Flag to show/hide plot diagrams.
	raw_data_display_flag = False
	band_pass_display_flag = False
	fft_display_signal_flag = False
	fft_display_resistance_a_flag = True
	fft_display_resistance_b_flag = True
	...
	for file_path in edf_list:
		...
		df_data_fft_total = pd.concat(
			[df_data_fft_fp1_ave, df_data_fft_fp2_ave, data_frame_fp1_fp2],
			axis=1
		)
		
		# concat each dataframe.
		df_data_fft_total = pd.concat(
			[df_data_fft_total, df_data_fft_fp2_ave],
			ignore_index=True,
			axis=1
		)
